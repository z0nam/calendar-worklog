...
> ③ **무료 ChatGPT = 불가 확정**: 동일 기계 A/B(무료 vs Plus)에서 무료엔 Google Calendar 커넥터 자체가 없어 캘린더 쓰기 불가 → worklog 핵심 기능 못 돌림.
...
- **2026-05-29 확정: 무료 티어로는 불가.** 무료 ChatGPT엔 Google Calendar 커넥터가 없어(설정→앱·앱 디렉터리 모두 부재, Plus엔 있음) 캘린더 쓰기 자체가 안 된다 → 동료 배포는 **Plus+ 전제**.
