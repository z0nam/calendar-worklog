...
JI 동료가 calendar-worklog를 본인 워크플로에 도입할 때 읽는 1페이지.
...
모든 쓰기는 **계획표 → 사용자 승인 → 등록** 순서. 채팅에서 본 뒤 OK 해야 등록됨.

## 누가 만들었나 / 누구용인가

조남운(JI PM) 본인 워크플로 자동화로 시작. **현재는 JI 내부 동료용** — 4 캘린더 분류(과제/근태/업무/기타)와 근태 규칙이 녹아 있음. (Phase 3에서 회사 무관하게 일반화 예정.)
...
- **캘린더는 공개 노출 전제** (`calendar.namun.net` 등이 published). 캘린더에 기록되는 제목/설명은 외부 노출된다고 보고 작성됨.
...
- 애매하면 캘린더에 등록하지 말고 사용자에게 확인 받음(`확인 필요`).
