...
```
ERROR: Permission to jeju-inst/calendar-worklog.git denied to <user>.
...
```sh
gh issue create --repo jeju-inst/calendar-worklog --title "제목" --body "내용"
```

**② fork 후 PR** — 직접 고칠 때.

```sh
gh repo fork jeju-inst/calendar-worklog --remote=true --clone=false
git switch -c fix/무엇을-고치는지
git commit -am "무엇을 왜 고쳤는지"
git push fork HEAD
gh pr create --repo jeju-inst/calendar-worklog
```
