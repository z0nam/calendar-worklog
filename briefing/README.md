...
동기화할 때 일정관리봇 DM으로 **스냅샷 2건**을 남긴다 — 리스트=오늘의 작업판(교체됨),
스냅샷=append-only 히스토리:

- **마감 스냅샷** (리스트 지우기 **전**): 지난 리스트의 완료/미완료를 박제한다. 완료는
  `~취소선~`(`✅`), 미완료는 `⬜`. 제목 `📋 지난 할 일 마감 — ✅N / ⬜M`. 비우면 사라질
  완료 기록을 남기는 게 목적이라 `replace`에서만 낸다.
- **계획 스냅샷** (새 리스트 생성 **후**): 오늘 할 일 + `note` 설명 + 리스트 링크.
  제목 `📋 오늘 할 일 — M/D (요일)`, 각 항목 `N. 내용  ~M/D` + `↳ 설명`.

두 포맷 모두 날짜·항목·기한·완료여부가 파싱 가능하게 고정한다(**중장기: 전사 업무현황
집계의 소스**로 쓸 형태).
